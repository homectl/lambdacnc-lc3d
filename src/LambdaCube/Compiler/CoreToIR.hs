...
import           LambdaCube.Compiler.DeBruijn        as I (HasFreeVars (..),
                                                           Rearrange)
import qualified LambdaCube.Compiler.DeBruijn        as I (shiftFreeVars, up)
import           LambdaCube.Compiler.DesugaredSource (Lit (LFloat, LInt, LString),
                                                      SName,
                                                      Visibility (Hidden, Visible),
                                                      untick)
import           LambdaCube.Compiler.Infer           (makeCaseFunPars',
                                                      neutType')
import           LambdaCube.Compiler.Pretty          (Doc, PShow (..), hsep,
                                                      nest, parens, plainShow,
                                                      ppShow, shortForm, text,
                                                      tupled, vcat, (<$$>),
                                                      (<+>), (</>))
...
addLEq l x = modL l $ \sv -> maybe (let i = length sv in i `seq` (i, Map.insert x i sv)) (, sv) $ Map.lookup x sv
...
getFragFilter :: ExpTV -> (Maybe ExpTV, ExpTV)
...
getFragmentShader :: ExpTV -> ((Maybe ExpTV, Ty), ExpTV)
...
getPrim, getPrim', getPrim'' :: ExpTV -> ExpTV
...
compFrameBuffer :: ExpTV -> IR.ClearImage
...
compSemantic :: ExpTV -> IR.ImageSemantic
compSemantic = \case
  A0 "Depth"   -> IR.Depth
...
compAC :: ExpTV -> IR.AccumulationContext
...
compBF :: ExpTV -> IR.BlendingFactor
compBF x = case x of
  A0 "ZeroBF"                -> IR.Zero
  A0 "OneBF"                 -> IR.One
...
compLO :: ExpTV -> IR.LogicOperation
compLO x = case x of
  A0 "Clear"        -> IR.Clear
  A0 "And"          -> IR.And
...
compComparisonFunction :: ExpTV -> IR.ComparisonFunction
...
compBool :: ExpTV -> Maybe Bool
compBool x = case x of
  A0 "True"  -> Just True
  A0 "False" -> Just False
  x          -> Nothing

compFrag :: ExpTV -> IR.FragmentOperation
...
compAttributeValue (ETuple x) = checkLength $ map go x
  where
    ...
    checkLength l@((a,_):_) = if all (\(i,_) -> i == a) l
      then map snd l
      else error "FetchArrays array length mismatch!"
...
genGLSLs backend
    ...
    = ( -- vertex input
      ...
      , -- fragment shader code
        ...
        <> vertOutDecls "in"
        <> [shaderDecl "out" (text t) (text n) | backend == OpenGL33, (n, t) <- zip fragOutNames fragOuts]
        <> fragFuncs
        <> [mainFunc $
               fragVals
    ...
    fixFuncs us ns fsb (Map.toList -> fsa)
      ...
      where
        combine (fn, (_, ty, tys)) def = case tys of
            ...
            _ ->
                ( [shaderFunc ot n
                            (zipWith (<+>) (map (text . toGLSLType "45") tys) (map text funArgs))
                            [shaderReturn def]]
                , []
                )
          where
            ot = text $ toGLSLType "44" ty
            n = text fn
...
tellUniform :: (MonadWriter (a, b) m, Monoid b) => a -> m ()
tellUniform x = tell (x, mempty)

simpleExpr :: ExpTV -> Bool
simpleExpr = \case
...
mapVal :: (I.Exp -> I.Exp) -> ExpTV -> ExpTV
...
instance Subst I.Exp ExpTV where
    subst_ i0 dx x (ExpTV a at vs) =
        ExpTV (subst_ i0 dx x a)
              (subst_ i0 dx x at)
              (zipWith (\i -> subst_ (i0+i)
                       (I.shiftFreeVars i dx)
                  $ I.up i x{-todo: review-}) [1..] vs)

addToEnv :: a -> [a] -> [a]
addToEnv x xs = x: xs
mkEnv :: I.Rearrange c => [c] -> [c]
mkEnv = {-trace_ ("mk " ++ show (length xs)) $ -} zipWith I.up [1..]
...
getPats :: (Eq a, Num a, Show a) => a -> ExpTV -> Maybe ([ExpTV], ExpTV)
getPats 0 e = Just ([], e)
...
getTTuple :: ExpTV -> Maybe [ExpTV]
getTTuple (A1 "HList" l) = Just $ compList l
getTTuple _              = Nothing

getTuple :: ExpTV -> Maybe [ExpTV]
...
genHLSLs :: Traversable t => Backend -> t ExpTV -> ExpTV -> (Maybe ExpTV, ExpTV) -> (Maybe ExpTV, ExpTV) -> Maybe ExpTV -> ([[Char]], Uniforms, Doc, Doc)
genHLSLs backend
    rp                  -- program point size
    (ETuple ints)       -- interpolations
    (vert, tvert)       -- vertex shader
    ...
    = ( -- vertex input
      ...
      , -- fragment shader code
        ...
        <> ["};"]
        <> ["struct PS_OUT {"]
        <> [shaderDecl' (text t) (text n) | backend == OpenGL33, (n, t) <- zip fragOutNames fragOuts]
        <> ["};"]
        <> fragFuncs

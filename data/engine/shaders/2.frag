...
vec4 diffuseLight_3_Float_4(vec4 z0,vec4 z1,vec4 z2,vec3 z3) {
    return vec4 (((z3) * (max (dot (z1,normalize ((z2) - (z0))),0.0))).x
                ,((z3) * (max (dot (z1,normalize ((z2) - (z0))),0.0))).y
                ,((z3) * (max (dot (z1,normalize ((z2) - (z0))),0.0))).z
                ,1.0);
}
